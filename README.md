...
当前图谱可视化功能包含：节点展示（记忆条目）、节点连线（记忆关联）、节点拖拽、图谱缩放/平移、节点点击查看详情、图谱编辑（添加/删除节点和连线）。

- [ ] A: 只保留只读展示（节点+连线+点击查看详情），移除节点拖拽、图谱编辑、缩放/平移
- [ ] B: 保留只读展示+缩放/平移，移除节点拖拽和图谱编辑
- [ ] C: 保留只读展示+缩放/平移+节点拖拽，只移除图谱编辑
- [ ] D: 其他方案
...
- [ ] A: 每次用户发送消息时自动检索匹配（类似RAG）
- [ ] B: 仅在AI判断需要记忆时自动检索（由系统提示词控制）
- [ ] C: 用户首次打开对话时检索一次，后续不自动检索
- [ ] D: 其他方案
